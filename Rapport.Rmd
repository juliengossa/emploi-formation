...
knitr::opts_chunk$set(fig.asp=9/16) # changer pour changer globalement le format des fig
...
# Mettre un titre complet. Possibilité d'utiliser "subtitle: " dans l'entête yaml
# Possibilité de mettre les sources en note de bas de page avec "^[texte]", marche aussi pour les "Notes".
# Plutôt que "graphique ci dessous" utiliser "Figure X" (avec X étant le numéro de la figure)
# Mettre les sources sur les graphiques partout ou nulle part.
# possibilité d'utiliser les backquotes `VAR` pour les noms de variable
# Des NA inutiles sur certains graphiques

# "R pour le codage" : codage veut dire autre chose, préférer "traitement des données"
# 3.1 Citer la volumétrie des données dans le texte pourra aider le lecteur à mesurer l'ampleur des travaux

# 3.2 "La dotation financière par étudiant est donc fonction du budget accordé via l’effectif des étudiants ainsi que les négociations. Les effectifs sont donc plus efficaces que les parts pour connaître la charge de l’ESR et son fonctionnement d'où leur utilisation ici."
# Je propose : "La dotation financière par étudiant est donc devenu un rétro-calcul, dans lequel intervient l'effectif étudiant en valeur absolu. Ce dernier est donc plus pertinent que la part des étudiants pour saisir la charge de l'ESR."

# 3.5 "il me semblait alors intéressant de comparer nos effectifs de chômeurs et inactifs (horsétudiants) avec les effectifs de NEET proposés par Eurostat." : intéressant pour quoi ?

# 3.7 Vérifier les chiffres du nombre d'apprentis donnés dans le texte

# 3.8 Corriger l'échelle y pour mettre en millions

# 3.10 "Graphique" au lieu de "tableau"

# 3.12 Vérifier ACP/ACM (composante principale / correspondances multiples)

# 4 "Cette formation m’a également permis d’adopter assez facilement" assez facilement indique que ça n'a demandé aucun effort... Mieux enlever, ou mettre "naturellement" pour dire que c'estr le fruit naturel des efforts. Aussi, il faut prendre le temps de donner des reflexe démographique concrets (sinon c'est creux). Plus globalement sur cette partie, je trouve qu'on insiste pas assez sur la transmission des savoirs/compétences démographiques à des scientifiques d'autres disciplines, ce qui a obligé à struturer ces connaissances, et mieux les maitriser pour pouvoir les communiquer.
...
# Présentation de l'organisme et de ma mission
...
__Activité des jeunes français.es entre 16 et 24 ans, depuis 1968__
...
## Réalisation d'un traitement de données des Enquêtes Emploi datant de 1971 à 2020
...
## Pourquoi utiliser des effectifs et non des pourcentages pour réaliser les graphiques qui concernent l'ESR ?
...
Il faut ainsi savoir que les budgets des universités et de l'enseignement supérieur ont décroché des effectifs au début des années 2000, d'abord avec l'abandon des modèles d'allocation des moyens tels que SAN REMO (système analytique de répartition des moyens) de 1995 à 2005 qui alloue des moyens aux universités en fonction des coûts moyens constatés puis SYMPA (système de répartition des moyens à la performance et à l’activité) de 2009 à 2019 qui devait allouer environ 80% des crédits selon les effectifs des personnels et étudiants et les 20% restants en fonction de la performance de l'établissement (nombre de publications, taux de réussite, etc...), mais n'a jamais été utilisé.

Avec le passage aux RCE (Responsabilités et compétences élargies), les crédits sont maintenant allousés sur la base d'une négociation durant laquelle les universités doivent mettre en avant leurs performances pour disposer des moyens financiers dont elles ont besoin (citer source - https://blog.educpros.fr/julien-gossa/2019/09/22/quel-dialogue-de-gestion-pour-quelle-autonomie/). On a donc un décrochage entre les moyens financiers alloués à l'ESR et les effectifs de jeunes qui poursuivent leurs études dans l'enseignement supérieur.

Pour résumer, auparavant l'équation des moyens accordées aux universités était auparavant la suivante : $Budget = étudiants \times dotation.par.étudiant$ alors qu'elle est maintenant : $Dotation.par.étudiant = \frac{Budget}{étudiants}$.
...
## Aperçu de la répartition des jeunes de 15 à 29 ans selon le statut d'activité

### Représentation de l'activité des jeunes en effectifs
...
__Enfin, on remarque sur ce graphique également une certaine constance du nombre de jeunes chômeurs et inactifs sur toute la période 1971 - 2020 mais il reste difficile d'affirmer avec certitude d'où provient cette relative stabilité.__


__Les jeunes de 15 à 29 ans en France de 1971 à 2020 selon le statut d'activité__
...
Le graphique ci-dessous montre une diminution importante des effectifs de jeunes de 25 à 29 ans n'ayant aucun diplôme ou un diplôme du CAP/BEP sur la période, alors qu'ils constituaient les effectifs les plus importants en 1971. On remarque aussi une augmentation importante de l'effectif des bacheliers, des détenteurs du DNB  et des diplômés d'un bac+2 ou plus à partir des années 1990. On peut en conclure que les jeunes sont donc de plus en plus diplômés au cours du temps, ce graphique témoigne donc de la démocratisation de l'enseignement en France qui a permis à une plus large part de la population d'atteindre le niveau bac ou supérieur au bac. 
...
__Effectif des jeunes de 25 à 29 ans en France selon le niveau de diplôme en France__
```{r activite.30, out.width="100%",fig.align='center', fig.asp=9/16}
  plot_activite3(agemin=25, agemax = 29)
```

Le graphique ci-dessous nous informe sur la part que repésente chaque niveau de diplôme chez les jeunes de 25 à 29 ans. On remarque qu'alors que la part des diplômés d'un bac+2 ou plus représentaient environ 15% seulement de la population des jeunes de 25 à 29 ans en 1971, ce chiffre s'élève à près de 50% en 2020. De la même manière, les jeunes ayant un niveau bac représentaient environ 5% de la population en 1971 alors qu'ils représentent en 2020 environ 18% des jeunes de 25 à 29 ans. A l'inverse, les individus sans aucun diplôme sont passés de quasiment 50% des jeunes de 25 à 29 ans en 1971 à environ 8% en 2020. 

__Répartition des jeunes de 25 à 29 ans selon le niveau de diplôme en France__
...
<!-- And so what? Il manque ici une articulation avec le reste : pourquoi regarder  -->
<!-- le niveau de diplôme ? -->
<!-- Ca nécessite une petite phrase d'intro, et une bonne phrase de conclusion. -->
<!-- Quelque chose comme : Pour mettre en contexte la tansition emploi/formation chez  -->
<!-- les jeunes, il convient de regarder aussi le niveau de qualification finalement -->
<!-- obtenu par cette population. -->
<!-- -  -->
<!-- Contrairement au niveau d'inactivité relativement stable, on constate de grand  -->
<!-- changements en ce qui concerne le niveau de diplômation. En particulier,avec  -->
<!-- une réduction de la population sans diplôme et un accroissement de la population  -->
<!-- diplômée du supérieur. Nous avons donc une mutation éducative, mais sans réelle  -->
<!-- mutation de l'inactivité économique.  -->

## Vérification de l'effectif de jeunes au sein de nos données

<!-- C'est une observation ou c'est une vérification ?  -->
<!-- Si c'est une vérification, c'est plus simple de le dire : maintenant qu'on a  -->
<!-- identifié ce phénomène, il convient de vérifier si d'autres données le confirme. -->
<!-- C'est pourquoi nous allons utiliser telle et telle base de données pour vérifier telle  -->
<!-- et telle chose. -->
...
__Les NEET de 15 à 29 ans de 1971 à 2021 en France__
...
En exploitant cette base de données, les résultats étaient sensiblement les mêmes que ceux exposés par notre base de données initiale (voir annexe 4), nous avons donc choisi de continuer à exploiter cette dernière, en raison de son poids beaucoup plus faible. 
...
Le taux de NEET issu de nos données semble être supérieur d'environ 2 à 2,5 points de poucentages à celui affiché par Eurostat.
Ainsi, selon nos données, en 2005, le taux de NEET au sein des jeunes de 15 à 29 ans en France atteint environ 15,6% alors qu'il est "seulement" de 13,2% d'après le traitement de données de l'Eurostat. De la même manière, en 2015, le taux de NEET indiqué par nos données est d'environ 16,7%  alors qu'il est de 14,8% selon Eurostat, enfin, en 2020, ces chiffres atteignent respectivement 17,4% et 14,0%.
<!-- Ici, il faut moduler ce sureffectif en indiquant au moins en pourcentage ce  -->
<!-- qu'il représente (la représentation est trompeuse, ne partant pas de 0). -->

## Analyse de la place des jeunes apprentis et de leur potentiel impact sur les NEET de 1976 à aujourd'hui
...
```{r activite.46, out.width="100%",fig.align='center', fig.cap="Champ : Apprentis de 15 à 29 ans", fig.asp=9/16}
  plot_Apprentis()
```
...
__Les jeunes de 18 à 21 ans de 1975 à 2020 selon leur statut d'activité__
...
```{r activite.48, out.width="100%",fig.align='center', fig.cap="Champ : Apprentis de 18 à 21 ans", fig.asp=9/16}
...
## Impact du changement de définition de l'activité de 2003 sur le statut d'activité des individus entre 30 et 60 ans 
...
__Effectif des individus de 30 ans à 60 ans de 1971 à 2020 selon leur statut d'activité__
...
__Effectif des jeunes étudiants ou suivant une formation de 15 à 29 ans entre 1971 et 2020 en France__
...
__Effectif des jeunes actifs occupés de 15 à 29 ans entre 1971 et 2020 en France__
...
__Effectif des chômeurs de 15 à 29 ans de 1971 à 2020 en France__
...
## Visualisation plus fine de la transition formation-emploi à l'aide de tableaux croisés
...
__Finalement l'ensemble de ces ACM a avant tout montré le changement de la place des femmes dans la société productive, passant globalement d'inactives avec un assez faible niveau de diplôme en raison du modèle familial traditionnel à actives occupées avec un niveau de diplôme en moyenne plus élevé que celui des hommes.__

<!-- Il manque ici une conclusion globale sur les informations obtenues par les travaux.-->
<!-- Par exemple ce qu'on trouve en conclusion après. -->


# Evaluation et analyse personnelle
...
CPESR : Conférence des Praticiens de l’Enseignement Supérieur et de la Recherche.
...
SAN REMO : Système Analytique de Répartition des Moyens.

SAPHIR : base historique des Recensements de Population sur la période 1968 – 2018. 
...
__Annexe 1 : Organigramme du laboratoire SAGE__
```{r annexe.1, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/organigrammeSAGE.PNG")
```


```{r annexe.2, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Calendrier.png")
```

__Annexe 3 : Représentation des effectifs des jeunes de 15 à 29 ans entre 1971 et 2020 en France__
```{r annexe.3, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Graphique_jeunes_Annexe.png")
```

__Annexe 4 : Représentation de l'effectif des NEET de 15 à 29 ans entre 1976 et 2020 avec une base de données classique__
```{r annexe.4, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/plot_NEETPoids.PNG")
```


__Annexe 5 : Représentation des effectifs de jeunes de 15 à 30 ans entre 1971 et 2020 selon le statut d'activité via l'application Shiny__
```{r annexe.5, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Shiny_one.PNG")
```


__Annexe 6 : Représentation des effectifs de jeunes selon chaque âge et le statut d'activité en 1971 et 2020 via l'application Shiny__
```{r annexe.6, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Shiny_three.PNG")
```

__Annexe 7 : Effectifs de jeunes de 15 à 30 ans en focntion du niveau de diplôme en 1971 et 2020__
```{r annexe.7, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Shiny_3.PNG")
```


__Annexe 8 : Représentation des effectifs de jeunes selon chaque âge et le niveau de diplôme en 1971 et 2020 via l'application Shiny__
```{r annexe.8, out.width="100%",fig.align='center', fig.asp=9/16}
  knitr::include_graphics("Images/Annexes/Shiny_4.PNG")
```
