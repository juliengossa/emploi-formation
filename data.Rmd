...
read_and_sum_71_74 <- function(file, annee) {
  ...
  Empl %>%
    group_by(Annee = annee,
             ...
             Diplome = fct_recode(dip,!!!var_recode_DIP)
    ) %>%
...
df <- read_and_sum_82_89("data/Csv/empl82qi.dta", 1982)
```
...
read_and_sum_2003_ <- function(file, annee=NA) {
  ...
  indiv %>%
    group_by(Annee = as.numeric(ANNEE),
             ...
             Apprentissage = fct_recode(as.character(STATUTR), !!!var_recode_STATUTR),
             #Diplome = fct_recode(as.character(DIP), !!!var_recode_DIP)
...
emploi <- emploi.raw %>%
  ...
  mutate(
    AgeQ = cut(Age,seq(0,120,5),labels=seq(0,115,5),right=FALSE)
  ) %>%
